...
def _settle_one(bet: dict, scores_by_id: dict) -> None:
    ...
    bet["payout"] = round(bet["stake"] * bet["odds_used"], 2) if won else 0.0
...
def settle_pending_days(ledger: dict, odds_api_key: str) -> None:
    ...
    for day in ledger["days"]:
        ...
        if not any(b["status"] == "pending" for b in day["bets"]):
            ...
            day["day_total_payout"] = round(sum((b["payout"] or 0) for b in resolved), 2)
            day["day_profit"] = round(day["day_total_payout"] - sum(b["stake"] for b in resolved), 2)
            day["settled"] = True
...
def add_new_day(ledger: dict, date_str: str, stakes: list, budget: float) -> None:
    ...
    ledger["days"].append({
        ...
        "budget": budget,
        "bets": stakes,
        "day_total_staked": round(sum(s["stake"] for s in stakes), 2),
        "day_total_payout": None,
        "day_profit": None,
        "settled": len(stakes) == 0,  # une journée sans pari qualifié est réglée d'office à 0
    })
